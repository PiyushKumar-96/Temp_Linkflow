...
import { Sparkles, Image as ImageIcon, Calendar, CheckCircle2, Clock, Loader2, ChevronDown, ChevronUp, X, Plus } from 'lucide-react';
...
function generateMockPost(topic: string, index: number, scheduleDate: string, scheduleTime: string): GeneratedPost {
...
export default function AIGeneratorShell() {
  ...
  const handleGenerate = async () => {
    ...
    for (let i = 0; i < postCount; i++) {
      const postTopic = customTopics.length > 0
  ...
  const handleScheduleAll = () => {
    setGeneratedPosts(prev => prev.map(p => ({ ...p, status: 'scheduled' as const })));
    setScheduledCount(generatedPosts.length);
    toast.success(`${generatedPosts.length} posts scheduled to Content Library & Calendar!`);
  };

  const handleScheduleOne = (id: string) => {
    setGeneratedPosts(prev => prev.map(p => p.id === id ? { ...p, status: 'scheduled' as const } : p));
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">· Generate, schedule, and publish LinkedIn posts with AI</span>
      </div>
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-2 flex flex-col gap-4">
          <div className="card p-5 flex flex-col gap-4">
            <h2 className="text-sm font-700 text-foreground uppercase tracking-wide">Generation Settings</h2>
              ...
              {customTopics.length > 0 && (
                <div className="flex flex-col gap-1 mt-1">
                  {customTopics.map((t, i) => (
                    <div key={i} className="flex items-center justify-between px-2.5 py-1.5 bg-muted rounded-lg">
                      <span className="text-xs text-foreground truncate">{t}</span>
                      <button onClick={() => removeCustomTopic(i)} className="text-muted-foreground hover:text-danger ml-2">
                        <X size={11} />
                      </button>
                    </div>
                  ))}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-lg font-700 text-primary w-8 text-center tabular-nums">{postCount}</span>
              </div>
            ...
            <div className="flex items-center justify-between py-2 border-t border-border">
              ...
              >
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${generateImages ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </div>
            </div>
          </div>
          ...
          <div className="card p-5 flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-700 text-foreground uppercase tracking-wide">Schedule Settings</h2>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${useScheduleRules ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </div>
            </div>
            ...
            {useScheduleRules && (
              <div className="flex items-start gap-2 px-3 py-2.5 bg-primary/5 border border-primary/20 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground">Posts will be scheduled using your Mon–Sun ruleset defined in Settings. Each post gets the next available slot.</p>
              </div>
            )}
          </div>
        ...
        <div className="col-span-3 flex flex-col gap-4">
          {generatedPosts.length === 0 && !isGenerating && (
            <div className="card flex flex-col items-center justify-center py-20 gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Configure your settings and click Generate</p>
              </div>
            </div>
          )}
          ...
          {isGenerating && (
            <div className="card flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-xs text-muted-foreground">Generating content, hashtags{generateImages ? ', and images' : ''}</p>
            </div>
          )}
          ...
          {generatedPosts.length > 0 && (
            <>
              {/* Bulk actions */}
              <div className="flex items-center justify-between">
                <p className="text-sm font-600 text-foreground">{generatedPosts.length} posts generated · {scheduledCount} scheduled</p>
                {readyCount > 0 && (
                  <button onClick={handleScheduleAll} className="btn-primary text-sm flex items-center gap-1.5">
                    <Calendar size={14} />
              ...
              <div className="flex flex-col gap-3">
                {generatedPosts.map((post, idx) => (
                  <div key={post.id} className={`card overflow-hidden transition-all ${post.status === 'scheduled' ? 'border-success/30 bg-success/5' : ''}`}>
                    <div className="flex items-start gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-700 text-muted-foreground">Post {idx + 1}</span>
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {post.hashtags.slice(0, 3).map(h => (
                            <span key={h} className="text-xs text-primary/70 font-500">{h}</span>
                          ))}
                        </div>
                      </div>
                    ...
                    {expandedPost === post.id && (
                      <div className="px-4 pb-4 border-t border-border pt-3 flex flex-col gap-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {post.hashtags.map(h => (
                            <span key={h} className="text-xs text-primary font-500">{h}</span>
                          ))}
                        </div>
                        {generateImages && (
                          <img
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={e => setGeneratedPosts(prev => prev.map(p => p.id === post.id ? { ...p, scheduledDate: e.target.value } : p))}
                            className="input-base text-xs py-1.5 flex-1"
                          />
                          <input
                            type="time"
                            value={post.scheduledTime}
                            onChange={e => setGeneratedPosts(prev => prev.map(p => p.id === post.id ? { ...p, scheduledTime: e.target.value } : p))}
