...
import { Check, X, Send, Clock, RefreshCw } from 'lucide-react';
        ...
        <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                <div className="w-5 h-5 rounded-full gradient-primary flex items-center justify-center">
                  <span className="text-white text-xs font-700" style={{ fontSize: 9 }}>
                    {post.authorInitials}
                  </span>
                </div>
                <span>
          ...
          {post.status === 'pending' && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                {isRejecting ? <RefreshCw size={14} className="animate-spin" /> : <X size={14} />}
                Reject
              </button>
              <button
              ...
              {post.hashtags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {post.hashtags.map((tag) => (
                    <span key={`tag-${tag}`} className="text-xs text-primary font-500">
                      {tag}
                    </span>
                  ))}
                </div>
                  ...
                  <div key={c.id} className={`rounded-lg p-3 ${commentTypeColors[c.type]}`}>
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className="w-6 h-6 rounded-full gradient-primary flex items-center justify-center">
                        <span className="text-white font-700" style={{ fontSize: 9 }}>
                          {c.authorInitials}
                        </span>
                      </div>
                      ...
                      {commentTypeLabels[c.type] && (
                        <span className="text-xs font-600 ml-auto text-muted-foreground">
                          {commentTypeLabels[c.type]}
                        </span>
                      )}
                    </div>
