...
export default function CalendarHeader({ view, onViewChange, currentDate, onNavigate, filterMember, onFilterChange }: Props) {
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
          ...
          {(['month', 'week'] as CalendarView[]).map(v => (
            <button
              ...
              className={`px-3 py-1.5 text-sm font-500 rounded-md capitalize transition-all duration-150 ${
                view === v ? 'bg-card text-foreground card-shadow' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              {v}
            </button>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => navigate(-1)} className="p-1.5 rounded-lg border border-border bg-card hover:bg-muted text-muted-foreground transition-colors">
            <ChevronLeft size={15} />
          </button>
          <span className="text-sm font-600 text-foreground px-2 min-w-[180px] text-center">{title}</span>
          <button onClick={() => navigate(1)} className="p-1.5 rounded-lg border border-border bg-card hover:bg-muted text-muted-foreground transition-colors">
