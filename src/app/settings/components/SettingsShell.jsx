    ...
    <div className="flex flex-col gap-6 max-w-4xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Used as default when applying offset rules
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card p-5 flex flex-col gap-5">
        ...
        <div className="flex items-start gap-2 px-3 py-2.5 bg-primary/5 border border-primary/20 rounded-lg">
          ...
          <p className="text-xs text-muted-foreground">
            Set posting times for each day. Use the offset rule to automatically calculate a
            day&apos;s time based on the previous day&apos;s first slot plus X minutes.
          </p>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 px-4 py-3 border-b border-border/60">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${rule.enabled ? 'translate-x-4' : 'translate-x-0.5'}`}
                    />
                  </div>
                  <span
                    className={`text-sm font-700 w-24 ${rule.enabled ? 'text-foreground' : 'text-muted-foreground'}`}
                  >
                    {rule.day}
                  </span>
                </label>
                      ...
                      <div className="flex items-center gap-2">
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          ...
                          <span className="text-xs text-muted-foreground">
                            Offset from {DAYS[idx - 1].slice(0, 3)}
                          </span>
                        </label>
                        {rule.useOffset && (
                          <>
                            <input
                              ...
                              onChange={(e) =>
                                updateRule(idx, { offsetFromPrev: Number(e.target.value) })
                              }
                              min={-1440}
                              max={1440}
                ...
                <div className="px-4 py-3 flex flex-wrap gap-2">
                  {rule.slots.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">
                      No time slots — click &quot;Add time&quot; to add one
                    </p>
                  ) : (
                    rule.slots.map((slot) => (
                      <div
                        key={slot.id}
                        className="flex items-center gap-1.5 bg-muted rounded-lg px-2 py-1.5"
                      >
      ...
      <div className="card p-5 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-7 gap-2">
          {rules.map((rule) => (
            <div
              key={`preview-${rule.day}`}
              className={`rounded-lg p-3 text-center ${rule.enabled ? 'bg-primary/5 border border-primary/20' : 'bg-muted/40 border border-border'}`}
            >
              <p
                className={`text-xs font-700 mb-2 ${rule.enabled ? 'text-primary' : 'text-muted-foreground'}`}
              >
                {rule.day.slice(0, 3)}
              </p>
              {rule.enabled && rule.slots.length > 0 ? (
                rule.slots.map((s) => (
                  <p key={s.id} className="text-xs font-600 text-foreground tabular-nums">
                    {s.time}
                  </p>
                ))
              ) : (
