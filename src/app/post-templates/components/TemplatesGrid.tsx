...
import type { Template } from './TemplatesShell';
...
export default function TemplatesGrid({ templates, onEdit, onDelete, onDuplicate, onUse }: Props) {
        ...
        return (
          ...
          >
                ...
                <div className="relative">
                  ...
                  {menuId === tpl.id && (
                    <div className="absolute right-0 top-6 z-20 w-36 bg-card border border-border rounded-lg card-shadow-md py-1 fade-in">
                      <button onClick={() => { onEdit(tpl); setMenuId(null); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-foreground hover:bg-muted transition-colors">
                        <Pencil size={12} /> Edit
                      </button>
                      <button onClick={() => { onDuplicate(tpl); setMenuId(null); toast.success('Template duplicated'); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-foreground hover:bg-muted transition-colors">
                        <Copy size={12} /> Duplicate
                      </button>
                      {!tpl.isDefault && (
                        <button onClick={() => { onDelete(tpl.id); setMenuId(null); toast.success('Template deleted'); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-danger hover:bg-danger/5 transition-colors">
            ...
            <div className="mx-4 mb-3 p-3 bg-muted/40 rounded-lg">
              <p className={`text-xs text-foreground leading-relaxed whitespace-pre-wrap font-mono ${isExpanded ? '' : 'line-clamp-4'}`}>
                {tpl.body}
              </p>
              {tpl.body.split('\n').length > 5 && (
                <button
            ...
            {tpl.hashtags.length > 0 && (
              <div className="px-4 pb-3 flex flex-wrap gap-1">
                {tpl.hashtags.map(tag => (
                  <span key={`htag-${tpl.id}-${tag}`} className="text-xs text-primary font-500">{tag}</span>
                ))}
              </div>
            )}
            ...
            <div className="mt-auto px-4 py-3 border-t border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className={`text-xs font-500 px-2 py-0.5 rounded-full capitalize ${toneColors[tpl.tone]}`}>
                  {tpl.tone}
                </span>
