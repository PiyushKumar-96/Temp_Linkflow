...
const suggestedTimes = [
  { id: 'slot-thu-noon', label: 'Thu 12:00 PM', score: '🔥 Best', date: '2026-09-10', time: '12:00' },
  { id: 'slot-wed-10', label: 'Wed 10:00 AM', score: '⚡ Great', date: '2026-09-09', time: '10:00' },
  { id: 'slot-fri-2', label: 'Fri 2:00 PM', score: '✓ Good', date: '2026-09-11', time: '14:00' },
];

interface Props {
  onClose: () => void;
  onSchedule: (date: string, time: string, assignee: string) => void;
}

export default function ScheduleDrawer({ onClose, onSchedule }: Props) {
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-card border-l border-border h-full flex flex-col slide-up shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-5 scrollbar-thin">
          {/* Suggested times */}
          <div>
            <label className="text-xs font-600 text-muted-foreground uppercase tracking-wide block mb-2">AI Recommended Times</label>
                ...
                <button
                  ...
                  className={`flex items-center justify-between px-3 py-2.5 rounded-lg border text-sm transition-all duration-150 ${
                    date === slot.date && time === slot.time
                      ? 'border-primary/40 bg-primary/5 text-primary' :'border-border bg-muted/30 text-foreground hover:bg-muted'
                  }`}
                >
          ...
          {/* Custom date/time */}
          <div>
            <label className="text-xs font-600 text-muted-foreground uppercase tracking-wide block mb-2">Custom Date & Time</label>
          ...
          {/* Recurrence */}
          <div>
            <label className="text-xs font-600 text-muted-foreground uppercase tracking-wide block mb-2">Recurrence</label>
            <select
              value={recurrence}
          ...
          {/* Assign reviewer */}
          <div>
            <label className="text-xs font-600 text-muted-foreground uppercase tracking-wide block mb-2">Assign Reviewer</label>
              ...
              {teamMembers.map(m => (
                <button
                  ...
                  className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-all duration-150 ${
                    assignee === m.id
                      ? 'border-primary/40 bg-primary/5' :'border-border bg-muted/30 hover:bg-muted'
                  }`}
                >
        ...
        <div className="px-5 py-4 border-t border-border flex gap-2">
          ...
          <button
            onClick={() => onSchedule(date, time, assignee)}
