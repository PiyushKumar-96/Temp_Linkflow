...
const categories = [
  'Thought Leadership', 'Case Study', 'Product Update',
  'Hiring', 'Event', 'Engagement', 'Company News', 'Industry Insight',
];
...
export default function ComposerEditor({
  content, onChange, selectedTone, onToneChange,
  hashtags, onHashtagsChange, category, onCategoryChange,
  imageUrl, onImageChange, isGenerating, onAIGenerate
}: Props) {
    ...
    <div className="card flex flex-col gap-0 overflow-hidden">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-border flex flex-wrap gap-3">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-600 text-muted-foreground uppercase tracking-wide">Tone</label>
      ...
      <div className="relative">
        ...
        <textarea
          value={content}
          onChange={e => !isAtLimit || e.target.value.length < content.length ? onChange(e.target.value.slice(0, MAX_CHARS)) : undefined}
      ...
      {hashtags.length > 0 && (
        <div className="px-4 pb-3 flex flex-wrap gap-1.5">
          {hashtags.map(tag => (
            ...
            >
              {tag}
              <button onClick={() => removeHashtag(tag)} className="hover:text-danger transition-colors">
                <X size={10} />
              </button>
            </span>
          ))}
      ...
      <div className="px-4 py-3 border-t border-border flex items-center justify-between bg-muted/30">
        ...
        <span className={`text-xs font-600 tabular-nums ${isAtLimit ? 'text-danger' : isNearLimit ? 'text-warning' : 'text-muted-foreground'}`}>
