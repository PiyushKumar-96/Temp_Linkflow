...
export default function ComposerPreview({ content, hashtags, imageUrl, isGenerating }: Props) {
  ...
  return (
    <div className="card p-4 flex flex-col gap-3 sticky top-20">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">Live preview</span>
      </div>
      ...
      <div className="border border-border rounded-xl overflow-hidden bg-white">
        ...
        <div className="px-3 pb-2">
          {isGenerating ? (
            <div className="space-y-2">
              {[100, 90, 95, 80, 85].map((w, i) => (
                <div key={`skel-line-${i}`} className="animate-pulse bg-gray-100 rounded h-3" style={{ width: `${w}%` }} />
              ))}
            </div>
          ) : content ? (
          ...
          {hashtags.length > 0 && content && (
            <div className="mt-2 flex flex-wrap gap-1">
              {hashtags.map(tag => (
                <span key={`prev-${tag}`} className="text-xs text-blue-600 font-500">{tag}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {imageUrl && (
          <div className="border-t border-gray-100">
            <img src={imageUrl} alt="LinkedIn post image" className="w-full object-cover max-h-56" />
          </div>
        )}
        ...
        <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-300 ${
              content.length > 2700 ? 'bg-danger' :
              content.length > 2400 ? 'bg-warning' : 'bg-primary'
            }`}
